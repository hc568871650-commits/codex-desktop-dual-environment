...
统一入口检查的是版本与文件完整性：同版缺文件或缺控制面板桌面入口会修复，完整时直接打开。无法解析或目录不匹配的安装清单会阻止自动覆盖，旧包不会覆盖更新版本。识别范围和手动参数见 [AUTO-SETUP.md](AUTO-SETUP.md)。
...
0.3.1 自动升级补建的桌面快捷方式也记入回滚记录，回滚会移除仍未修改的新增入口；若用户后来修改了它，则停止回滚并保留。安装位置登记只用于下次发现目录，不作为允许覆盖的依据，卸载会移除仍匹配本安装的位置登记。
